...
import { 
  ArrowLeft, MapPin, Star, Calendar, AlertCircle, 
  Phone, Mail, Clock, CheckCircle, XCircle, Globe, Info
} from "lucide-react";
...
export default function AgencyDetailsPage() {
  ...
  useEffect(() => {
    if (params?.id) {
      ...
      const found = allAgencies.find((a) => a.id === id);
  ...
  if (!agency) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50">
        ...
        <Link href="/Agencies" className="text-blue-600 hover:underline mt-4 font-bold">
          Retour aux agences
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50 pb-20 font-sans text-slate-800">
      ...
      <main className="max-w-[1440px] mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-4 space-y-8">
            ...
            <div className="relative z-10">
                ...
                <div className="space-y-4 text-left bg-slate-50 p-5 rounded-2xl">
                    <div className="flex items-center gap-3 text-sm">
                        ...
                        <span className="font-semibold text-slate-700">{agency.phone}</span>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
                        <div className="p-2 bg-white rounded-full text-blue-600 shadow-sm"><Mail size={16} /></div>
                        <span className="font-semibold text-slate-700 truncate">{agency.email}</span>
                    </div>
                    <div className="flex items-center gap-3 text-sm">
          ...
          {/* Section Localisation (NOUVEAU : CARTE AJOUTÉE) */}
          <div className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100">
            ...
            <div className="bg-slate-100 rounded-3xl h-64 relative overflow-hidden w-full group border border-slate-200">
                <iframe 
                  ...
                  // On combine la localisation et la ville pour une meilleure précision
                  src={`https://maps.google.com/maps?q=${encodeURIComponent(agency.location + ', ' + agency.city)}&t=&z=15&ie=UTF8&iwloc=&output=embed`}
          ...
          <div className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100">
            ...
            <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl">
                ...
                <span className="font-bold text-slate-800">{agency.openingHours}</span>
            </div>
            <div className={`mt-4 flex items-center justify-center gap-2 py-2 rounded-xl font-bold text-sm ${agency.isOpen ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                {agency.isOpen ? <CheckCircle size={16} /> : <XCircle size={16} />}
                {agency.isOpen ? "Ouvert actuellement" : "Fermé actuellement"}
            </div>
          </div>
             ...
             <div className="absolute bottom-6 left-8 text-white">
                <div className="flex items-center gap-2 mb-2">
                    <div className="flex text-yellow-400">
                        {[...Array(5)].map((_, i) => (
                            <Star key={i} size={18} className={i < Math.round(agency.rating) ? "fill-current" : "text-white/30"} />
                        ))}
                    </div>
                    <span className="font-bold text-lg">{agency.rating}</span>
                    <span className="text-sm opacity-80">({agency.reviewCount} avis)</span>
                </div>
                <p className="font-medium text-white/90 flex items-center gap-2">
                    <MapPin size={18} /> {agency.location}
                </p>
             </div>
          </div>
            ...
            <div className="p-8 bg-slate-50/30 h-full flex-1">
                    ...
                    <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6">
                        ...
                        <ul className="space-y-2">
                            <li className="flex items-center gap-2 text-slate-700 text-sm">
                                <CheckCircle size={16} className="text-blue-500" /> Véhicules inspectés et certifiés
                            </li>
                            <li className="flex items-center gap-2 text-slate-700 text-sm">
                                <CheckCircle size={16} className="text-blue-500" /> Assistance 24/7 incluse
                            </li>
                            <li className="flex items-center gap-2 text-slate-700 text-sm">
                                <CheckCircle size={16} className="text-blue-500" /> Annulation flexible
                            </li>
                        </ul>
                    </div>
                  </div>
                ...
                {activeTab === "avis" && (
                  <div className="space-y-4">
                    {agency.reviews.length > 0 ? agency.reviews.map((review: any) => (
